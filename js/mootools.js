...
function _merge() {
	...
	for ( var i = 0; i < arguments.length; i++) {
		for ( var property in arguments[i]) {
			...
			var mp = mix[property];
			if (mp && _type(ap) == 'object' && _type(mp) == 'object')
				mix[property] = _merge(mp, ap);
			else
				mix[property] = ap;
		}
	}
	...
	for ( var i = 0, l = arguments.length; i < l; i++) {
		arguments[i].extend = function(props) {
			for ( var prop in props) {
				...
				if (!this[prop])
					this[prop] = _native.generic(prop);
			}
		};
	}
};
...
function _pick(obj, picked) {
	return _defined(obj) ? obj : picked;
};
...
var Class = function(properties) {
	var klass = function() {
		return (arguments[0] !== null && this.initialize && _type(this.initialize) == 'function') ? this.initialize
...
Class.prototype = {
	...
	implement : function() {
		for ( var i = 0, l = arguments.length; i < l; i++)
			_extend(this.prototype, arguments[i]);
	}
};
Class.Merge = function(previous, current) {
	if (previous && previous != current) {
		var type = _type(current);
		if (type != _type(previous))
			return current;
		switch (type) {
		case 'function':
			var merged = function() {
...
var Events = new Class({
	...
	removeEvent : function(type, fn) {
		if (this._events && this._events[type])
			this._events[type].remove(fn);
		return this;
	}
});
var Options = new Class({
	setOptions : function() {
		this.options = _merge.apply(null, [ this.options ].extend(arguments));
		if (this.addEvent) {
			for ( var option in this.options) {
				if (_type(this.options[option] == 'function')
						&& (/^on[A-Z]/).test(option))
...
Array
		.extend({
			...
			getRandom : function() {
				return this[_random(0, this.length - 1)] || null;
			},
			getLast : function() {
...
function _each(iterable, fn, bind) {
	if (iterable && typeof iterable.length == 'number'
			&& _type(iterable) != 'object') {
...
String.extend({
	test : function(regex, params) {
		return ((_type(regex) == 'string') ? new RegExp(regex, params) : regex)
				.test(this);
	},
	toInt : function() {
	...
	hexToRgb : function(array) {
		var hex = this.match(/^#?(\w{1,2})(\w{1,2})(\w{1,2})_/);
...
Function.extend({
	create : function(options) {
		...
		if (_chk(options.arguments) && _type(options.arguments) != 'array')
		...
		return function(event) {
			...
			var returns = function() {
				return fn.apply(_pick(options.bind, fn), args);
			};
			if (options.delay)
...
var Elements = new Class({
	initialize : function(elements) {
		return (elements) ? _extend(elements, this) : this;
	}
});
...
function __() {
	...
	return __.unique(elements);
};
__.unique = function(array) {
	var elements = [];
	for ( var i = 0, l = array.length; i < l; i++) {
		if (array[i]._included)
			continue;
		var element = _(array[i]);
		if (element && !element._included) {
			element._included = true;
			elements.push(element);
		}
	}
	for ( var n = 0, d = elements.length; n < d; n++)
		elements[n]._included = null;
...
Elements.Multi = function(property) {
	return function() {
		...
		for ( var i = 0, j = this.length, returns; i < j; i++) {
			...
			if (_type(returns) != 'element')
				elements = false;
			items.push(returns);
		}
		;
		return (elements) ? __.unique(items) : items;
	};
};
Element.extend = function(properties) {
	for ( var property in properties) {
		...
		Element[property] = _native.generic(property);
...
Element.extend({
	...
	clone : function(contents) {
		var el = _(this.cloneNode(contents !== false));
		if (!el._events)
			return el;
		el._events = {};
		for ( var type in this._events)
			el._events[type] = {
				'keys' : _A(this._events[type].keys),
				'values' : _A(this._events[type].values)
			};
	...
	removeClass : function(className) {
		this.className = this.className.replace(
				new RegExp('(^|\\s)' + className + '(?:\\s|_)'), '_1').clean();
		return this;
	},
	...
	setOpacity : function(opacity) {
		...
		if (window.ie)
			this.style.filter = (opacity == 1) ? '' : "alpha(opacity="
					+ opacity * 100 + ")";
		this.style.opacity = this._tmp.opacity = opacity;
		return this;
	},
	...
	getParent : function() {
		return _(this.parentNode);
	},
	getChildren : function() {
		return __(this.childNodes);
	},
	hasChild : function(el) {
		return !!_A(this.getElementsByTagName('*')).contains(el);
	},
	...
	setHTML : function() {
		this.innerHTML = _A(arguments).join('');
		return this;
	},
	setText : function(text) {
		...
		this[_defined(this.innerText) ? 'innerText' : 'textContent'] = text;
		return this;
	},
	getText : function() {
		var tag = this.getTag();
		...
		return (_pick(this.innerText, this.textContent));
	},
	getTag : function() {
...
var Garbage = {
	...
	trash : function(elements) {
		for ( var i = 0, j = elements.length, el; i < j; i++) {
			if (!(el = elements[i]) || !el._tmp)
				continue;
			if (el._events)
				el.fireEvent('trash').removeEvents();
			for ( var p in el._tmp)
				el._tmp[p] = null;
			for ( var d in Element.prototype)
				el[d] = null;
			Garbage.elements[Garbage.elements.indexOf(el)] = null;
			el.htmlElement = el._tmp = el = null;
		}
...
Element.Methods.Events = {
	addEvent : function(type, fn) {
		...
		if (this._events[type].keys.contains(fn))
			return this;
		this._events[type].keys.push(fn);
		var realType = type;
		...
		this._events[type].values.push(fn);
	...
	removeEvent : function(type, fn) {
		if (!this._events || !this._events[type])
			return this;
		var pos = this._events[type].keys.indexOf(fn);
		if (pos == -1)
			return this;
		var key = this._events[type].keys.splice(pos, 1)[0];
		var value = this._events[type].values.splice(pos, 1)[0];
		var custom = Element.Events[type];
		if (custom) {
			if (custom.remove)
	...
	removeEvents : function(type) {
		if (!this._events)
			return this;
		if (!type) {
			for ( var evType in this._events)
				this.removeEvents(evType);
			this._events = null;
		} else if (this._events[type]) {
			this._events[type].keys.each(function(fn) {
	...
	cloneEvents : function(from, type) {
		if (!from._events)
			return this;
		if (!type) {
			for ( var evType in from._events)
				this.cloneEvents(from, evType);
		} else if (from._events[type]) {
			from._events[type].keys.each(function(fn) {
...
function _E(selector, filter) {
	return (_(filter) || document).getElement(selector);
};
function _ES(selector, filter) {
	return (_(filter) || document).getElementsBySelector(selector);
};
__.shared = {
	'regexp' : /^(\w*|\*)(?:#([\w-]+)|\.([\w-]+))?(?:\[(\w+)(?:([!*^_]?=)["']?([^"'\]]*)["']?)?])?_/,
	'xpath' : {
		...
		getItems : function(items, context, nocash) {
			...
			return (nocash) ? elements : new Elements(elements.map(_));
		}
	},
	'normal' : {
		getParam : function(items, context, param, i) {
			if (i == 0) {
				if (param[2]) {
					...
					items = _A(context.getElementsByTagName(param[1]));
				}
			} else {
				items = __.shared.getElementsByTagName(items, param[1]);
				if (param[2])
		...
		getItems : function(items, context, nocash) {
			return (nocash) ? items : __.unique(items);
		}
	},
...
__.shared.method = (window.xpath) ? 'xpath' : 'normal';
Element.Methods.Dom = {
	getElements : function(selector, nocash) {
		...
		for ( var i = 0, j = selector.length; i < j; i++) {
			var sel = selector[i];
			var param = sel.match(__.shared.regexp);
			if (!param)
				break;
			param[1] = param[1] || '*';
			var temp = __.shared[__.shared.method].getParam(items, this, param,
					i);
			if (!temp)
				break;
			items = temp;
		}
		return __.shared[__.shared.method].getItems(items, this, nocash);
	},
	getElement : function(selector) {
		return _(this.getElements(selector, true)[0] || false);
	},
	getElementsBySelector : function(selector, nocash) {
		...
		return (nocash) ? elements : __.unique(elements);
	}
};
Element
		.extend({
...
Element.extend({
	getValue : function() {
		...
		case 'select':
			var values = [];
			_each(this.options, function(option) {
				if (option.selected)
					values.push(_pick(option.value, option.text));
			});
	...
	getFormElements : function() {
		return __(this.getElementsByTagName('input'), this
	...
	add : function(fn) {
		...
		var domReady = function() {
			if (window.loaded)
				return;
			window.loaded = true;
			window.timer = _clear(window.timer);
		...
		if (document.readyState && window.webkit) {
			...
			if (!_('ie_ready')) {
				...
				document.write('<script id="ie_ready" defer src="' + src
						+ '"><\/script>');
				_('ie_ready').onreadystatechange = function() {
...
Drag.Base = new Class(
		{
			...
			initialize : function(el, options) {
				this.setOptions(options);
				this.element = _(el);
				this.handle = _(this.options.handle) || this.element;
				this.mouse = {
					'now' : {},
					'pos' : {}
				};
			...
			start : function(event) {
				...
				for ( var z in this.options.modifiers) {
					...
					if (limit && limit[z]) {
						for ( var i = 0; i < 2; i++) {
							if (_chk(limit[z][i]))
								this.limit[z][i] = (_type(limit[z][i]) == 'function') ? limit[z][i]
										()
										: limit[z][i];
						}
					}
				}
				if (_type(this.options.grid) == 'number')
					this.options.grid = {
						'x' : this.options.grid,
						'y' : this.options.grid
					};
			...
			drag : function(event) {
				...
				for ( var z in this.options.modifiers) {
					...
					if (this.limit[z]) {
						if (_chk(this.limit[z][1])
								&& (this.value.now[z] > this.limit[z][1])) {
							this.value.now[z] = this.limit[z][1];
							this.out = true;
						} else if (_chk(this.limit[z][0])
...
Element.extend({
	makeResizable : function(options) {
		return new Drag.Base(this, _merge({
			modifiers : {
				x : 'width',
				y : 'height'
			}
...
Drag.Move = Drag.Base
		.extend({
			...
			initialize : function(el, options) {
				this.setOptions(options);
				this.element = _(el);
				this.droppables = __(this.options.droppables);
				...
				if (this.position.element == 'absolute'
					...
					top = _chk(top) ? top : this.element
							.getTop(this.options.overflown);
					left = _chk(left) ? left : this.element
							.getLeft(this.options.overflown);
				} else {
					top = _chk(top) ? top : 0;
					left = _chk(left) ? left : 0;
				}
