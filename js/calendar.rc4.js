...
var Calendar = new Class({	
	...
	initialize: function(obj, options) {
		...
		for (var i in obj) {
			...
			_extend(cal, this.bounds(cal)); // abs bounds of calendar

			_extend(cal, this.values(cal)); // valid days, months, years
	...
	bounds: function(cal) {
		...
		if (this.options.direction > 0) {
			start = new Date();
			start.setDate(date + this.options.pad * cal.id - 1);
		}
		
		if (this.options.direction < 0) {
			end = new Date();
			end.setDate(date - this.options.pad * (this.calendars.length - cal.id - 1) + 1);
		}
			...
			if (el.getTag() == 'select') {		
				...
				if (el.format.test('(F|m|M|n)')) { // search for a month select
					...
					el.getChildren().each(function(option) { // get options
						...
						if (_type(values[0]) != 'number' || values[0] == years[0]) { // if it's a year / month combo for curr year, or simply a month select
							if (!months_start.contains(values[1])) { months_start.push(values[1]); } // add to months array
						}
	
						if (_type(values[0]) != 'number' || values[0] == years.getLast()) { // if it's a year / month combo for curr year, or simply a month select
	...
	changed: function(cal) {
		...
		_extend(cal, this.values(cal)); // update bounds - based on curr month
	...
	element: function(el, f, cal) {
		if (_type(f) == 'object') { // in the case of multiple inputs per calendar
			for (var i in f) { 
	...
	navigate: function(cal, type, n) {
		...
		_extend(cal, this.values(cal));

		if (_type(cal.months) == 'array') { // if the calendar has a months select
	...
	read: function(cal) {
		...
		cal.els.each(function(el) {
			...
			values.each(function(val, i) { 
				if (_type(val) == 'number') { arr[i] = val; }
			}); 
		}, this);

		// we can update the cals month and year values
		if (_type(arr[0]) == 'number') { cal.year = arr[0]; }
		if (_type(arr[1]) == 'number') { cal.month = arr[1]; }

		var val = null;

		if (arr.every(function(i) { return _type(i) == 'number'; })) { // if valid date
	...
	rebuild: function(cal) {
		cal.els.each(function(el) {			
			/*
			if (el.getTag() == 'select' && el.format.test('^(F|m|M|n)_')) { // special case for months-only select
			...
			*/
			...
			if (el.getTag() == 'select' && el.format.test('^(d|j)_')) { // special case for days-only select
	...
	values: function(cal) {
			...
			if (el.getTag() == 'select') {		
				...
				if (el.format.test('(F|m|M|n)')) { // search for a month select
					...
					el.getChildren().each(function(option) { // get options
						...
						if (_type(values[0]) != 'number' || values[0] == cal.year) { // if it's a year / month combo for curr year, or simply a month select
				...
				if (el.format.test('(d|j)') && !el.format.test('^(d|j)_')) { // search for a day select, but NOT a days only select
					days = []; // 1 - 31
		...
		if (_type(days) == 'array') { // somewhere there was a days select
